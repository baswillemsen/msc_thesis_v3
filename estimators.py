...
def arco(df: object, target_country: str, timeframe: str,
    ...
    if fake_num in list(target_diff):
        ...
        X_train, X_test, y_train, y_test = train_test_split(X_diff_stand, y_diff_stand, test_size=0.25,
        ...
        # summarize chosen configuration (difference)
        ...
        act_pred_diff = pd.DataFrame(list(zip(act_diff, pred_diff)),
                                     columns=['act_diff', 'pred_diff']).set_index(target_diff.index)
        shapiro_wilk_test(df=act_pred_diff, target_country=target_country, alpha=0.05)
        if save_results:
            act_pred_diff.to_csv(f'{tables_path_cor}{target_country}/{target_country}_arc_pred')
        ...
        orig_data = orig_data[(orig_data['country'] == target_country) &
                              (df['date'] >= date_start)].set_index('date')[target_var]
        ...
        act_pred = pd.DataFrame(list(zip(orig_data_log, pred2)), columns=['act', 'pred']).set_index(orig_data_log.index)
